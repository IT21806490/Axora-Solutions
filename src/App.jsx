...
// This allows them to be dynamically referenced by name.
const icons = {
  Rocket,
  Server,
  Lightbulb,
...
const App = () => {
  ...
  // Set up the Three.js background
  useEffect(() => {
    ...
    const init = () => {
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        color.setHSL(Math.random() * 0.1 + 0.5, 1.0, 0.5); // Blue-ish hues
        colors[i3] = color.r;
        colors[i3 + 1] = color.g;
        colors[i3 + 2] = color.b;
      }
  ...
  // Typing effect for hero text - now loops
  useEffect(() => {
    let i = 0;
    const typingInterval = setInterval(() => {
      if (i < fullText.length) {
        setHeroText(prevText => prevText + fullText.charAt(i));
        i++;
      } else {
  ...
  useEffect(() => {
    ...
    if (activeTab === 'services') {
      const timers = servicesData.map((_, index) => {
        return setTimeout(() => {
      ...
      const timers = internsData.map((_, index) => {
        return setTimeout(() => {
          setVisibleInterns(prev => ({ ...prev, [index]: true }));
        }, index * 150);
      });
      setVisibleCards({});
      setVisibleBlogPosts({});
      ...
      const timers = blogPostsData.map((_, index) => {
        return setTimeout(() => {
          setVisibleBlogPosts(prev => ({ ...prev, [index]: true }));
        }, index * 150);
      });
      setVisibleCards({});
      setVisibleInterns({});
      ...
      const timers = whyUsData.map((_, index) => {
        return setTimeout(() => {
          setVisibleWhyUs(prev => ({ ...prev, [index]: true }));
        }, index * 150);
      });
      setVisibleCards({});
      setVisibleInterns({});
  ...
  const InternCard = ({ title, description, requirement, icon: Icon, isVisible, index }) => (
    ...
    >
      ...
      <div className="flex justify-center">
        <a
          href={`https://wa.me/1234567890?text=${encodeURIComponent(`I'm interested in the ${title} internship!`)}`}
          target="_blank"
  ...
  // Corrected data to use icon names as strings to reference the icons object
  const blogPostsData = [
  ...
  return (
    <div className="bg-slate-900 text-gray-100 min-h-screen font-sans overflow-hidden">
      {/* 3D Background canvas, fixed to the viewport */}
      ...
      <div className="fixed top-0 left-0 right-0 z-50 bg-slate-900 bg-opacity-80 backdrop-blur-sm shadow-lg">
        <header className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row justify-between items-center">
          ...
          >
            ...
            <TabButton tab="blog" label="Blog" icon={PenSquare} />
            <TabButton tab="contact" label="Contact" icon={Mail} />
          </nav>
        </header>
      </div>
